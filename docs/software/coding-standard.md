...
::: tip
These rules are a bit of a boring read. When getting started, skip to the [example code](./coding-standard-example) and copy its style.
:::
...
Identifiers should be chosen to be as descriptive as possible, whilst not making the code too verbose. In particular, single character identifiers should only be used for loop variables and in mathematical code.
...
```cpp
units::degree_t motor_angle = 90_deg;
...
Files begin with a heading that contains the file name.
...
Functions should be documented directly above the function declaration in the header file. The documentation should include a summary, description of all parameters and the return value if any.
...
In some cases code may be incomplete, or require improvement, or checking, later. In such cases a comment should be included that begins with ‘TODO’ and details the issue. By always using ‘TODO’ and not alternatives like ‘FIXME’ or ‘HACK’, we can easily search for problems and make sure they have been dealt with. There should be no ‘TODO’s left in the code by the time we get to competition.
...
Include files should be ordered in 3 groups with a blank line between each group, as follows:
...
int really_long_variable = ReallyExceptionallyLongFunctionName(
    very_long_long_parameter, other_parameters);
```
...
Do not use spaces inside of any brackets (i.e. parentheses) or square brackets.

Do use spaces inside of braces.

### Magic Numbers

Code should not use magic numbers. Instead a constant should be defined to increase readability. The source of numbers in the code should be documented, even if only to record that the value was found by experimenting. The scope of the constant should be the class level if it applies to multiple functions, or if it represents an important tuneable parameter of the class, otherwise the scope of the constant should just be the function.
...
3. Any power of 10 when converting the scale of units, e.g. millimetres to metres

### No Multi-Statement Lines

All statements should be on a single line, except that a `break` statement can be on the same line in a single line `case` within a `switch` statement.
...
1. Filenames are meant to be lower case. Instead we have the filename being the same as the class (i.e. upper camel case). This seems more consistent and is the convention used by the example code in the Eclipse plugin.
2. We have a convention for static class data members (we prefix `ms_`, rather than just `m_`), interface classes and templates. This is a simple enhancement and follows conventions used elsewhere (e.g. Microsoft).
3. Accessors and mutators (aka getters and setters) are meant to use only lower case and follow the member variable name. We ignore this as it is clearly not being used by WPILib.
...
5. We are not using the 80 character line limit. Eclipse examples do not always do this, although they seem to mostly be close to it. This is very old school and often leads to wrapping code that would still fit on any screen someone is likely to be using.
6. We are using 4 character indenting not 2. Eclipse examples all use 4.
7. We are not indenting `public`, `protected` and `private` by one space. Eclipse samples do not do this.
