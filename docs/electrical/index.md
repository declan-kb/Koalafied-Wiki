# Electrical

## Reference

- [Control System Diagram](./control-system-diagram): how every part of the control system connects
- [Understanding DC Motors](./understanding-dc-motors): motor theory, current limits and gear ratio selection
- [Wire Gauge Guide](./wire-gauge-guide): game manual minimums, the gauges we use, and AWG sizes

## Skills Tree

This document serves as a companion to the ‘[Electrical Skills Tree](./skills-tree)’, and contains all of the information required to pass any knowledge-based components of each skill, and guides for practical components. The information is organised by skill, in the same order as the skills tree. This document is separate to the Electrical Systems Manual (to be published), serving as a revision sheet for new electrical students, rather than a comprehensive reference.

In general, the level of detail provided will be greater than is required for a pass on a specific skill requirement, so do not worry about memorising everything, but it should be clear you understand what you are talking about. Links to WPILib Docs pages will be included in some skills for further reading, although it is not required.

- [Skills Tree Overview](./skills-tree): prerequisites and requirements for every skill
- [Basic Electrical Knowledge](./basic-electrical-knowledge)
- [Basic Electrical Knowledge II](./basic-electrical-knowledge-ii)
- [Basic Soldering](./basic-soldering)
- [Power Soldering](./power-soldering)
- [Component Soldering](./component-soldering)
- [Common Component Connectors](./common-component-connectors)
- [Anderson Powerpole Connectors](./anderson-powerpole-connectors)
- [Specialised Component Connectors](./specialised-component-connectors)
- [Spade Connectors](./spade-connectors)
- [Deutsch DTM Connectors](./deutsch-dtm-connectors)
- [Basic Circuit Design](./basic-circuit-design)
- [Circuit Topologies](./circuit-topologies)
- [Belly/Brainpan Layout](./belly-brainpan-layout)
- [Wire Gauge Selection](./wire-gauge-selection)
- [Mechanism Wiring Layout](./mechanism-wiring-layout)
